...
fn test_write_and_read_back_simple() {
    let path = "tests/data/written_simple.h5";
...
fn test_write_multiple_datasets() {
    let path = "tests/data/written_multi.h5";
...

    {
        ...
        let vals1: Vec<f64> = raw1.chunks_exact(8).map(|c| f64::from_le_bytes(c.try_into().unwrap())).collect();
        assert_eq!(vals1, vec![1.0, 2.0, 3.0]);

        let ds2 = f.dataset("ints").unwrap();
        let raw2 = ds2.read_raw().unwrap();
        let vals2: Vec<i32> = raw2.chunks_exact(4).map(|c| i32::from_le_bytes(c.try_into().unwrap())).collect();
...
fn test_write_with_group() {
    let path = "tests/data/written_group.h5";

    {
        ...
        w.create_dataset("/subgroup", &DatasetSpec {
            name: "tiny",
            shape: &[1],
            dtype: DtypeSpec::U8,
            data: &data,
...
fn test_write_readable_by_h5dump() {
    let path = "tests/data/written_h5dump.h5";
    ...
    match output {
        Ok(out) => {
            ...
            assert!(stdout.contains("1"), "h5dump output should contain data values");
        }
        Err(e) => {
